...
import { getFarmsByCooperative, getMembersByCooperative } from '../../pages/cooperative/members-management/my-members/utils/getMembers';
...
// Add user already created to the cooperative 
export const addUserToCooperative = createAsyncThunk('users/addUser', async (data, { dispatch, getState }) => {
  const { cooperative } = getState().authSlice;
  dispatch(startLoading());
  try {
   await axios.post(`${import.meta.env.VITE_YVY_BACKEND_BASE_URL}/cooperatives/users/addUser`,data); 
  } catch (error) {
    dispatch(hasError(error));
  }
  await dispatch(getMembersByCooperative(cooperative.id));
});
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getMembersSuccess(state, action) {
        ...
        state.filteredMembers = action.payload.members;
        state.countries = action.payload.uniqueCountries;
        state.error = null;
    },
    ...
    resetFilteredMembers(state) {
      state.filteredMembers = state.farms;
      state.filterCountry = 'All';
    }
  },
});
