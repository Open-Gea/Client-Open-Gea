...
export const updateCalc = createAsyncThunk('calcs/newCalc', async (data, { dispatch, getState }) => {
  ...
  try {
    ...
    const response = await axios.post(
      `${import.meta.env.VITE_YVY_BACKEND_BASE_URL}/carbonFootPrint/calculate`,
      calcParams
    );
  ...
  try {
    ...
    await dispatch(getFarmsEvo(cooperative.id, false));
...
const slice = createSlice({
  name: 'evotranspiracionCooperativaSlice',
  initialState,
  reducers: {
    // LOADING
    startLoading(state) {
...
export const setCalc =
  (id, finishLoad = true) =>
  (dispatch, getState) => {
    const { farms } = getState().evotranspiracionCooperativaSlice;
