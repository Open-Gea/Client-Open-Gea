...
// sections
import { RequestsListHead, RequestsListToolbar } from './components';
// redux
import { useDispatch, useSelector } from 'react-redux';
// actions
import { handleChangeRowsPerPage, setPage } from '../../../../redux/slices/requestsCooperativa';
import { getRequestsByCooperative } from './utils/getRequests';
...
import RequestsMoreMenu from './components/RequestsMoreMenu';

// ----------------------------------------------------------------------

export function RequestsInformation() {
  const { i18n, t } = useTranslation('invites');
  const { t: tCommon } = useTranslation('common');
  const dispatch = useDispatch();
  const { page, rowsPerPage, order, orderBy, filterName, filteredRequestsCooperative, isLoading, error } = useSelector(state => state.requestsCooperativaSlice);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - filteredRequestsCooperative.length) : 0;
  let filtered = applySortFilter(filteredRequestsCooperative, getComparator(order, orderBy), filterName);
  if(stateRequests){
      // Filter by the state selected
      filtered = filtered.filter(function(invite){
          if(i18n.language === 'en') {
            if(stateRequests !== 'All Requests'){
              return  (invite.status[0].toLowerCase()) === (stateRequests[0].toLowerCase());
            }else{
              return true;
            }    
          }else{
            if(stateRequests !== 'Todas las solicitudes'){
              return (invite.status[0].toLowerCase()) === (stateRequests[0].toLowerCase());
            }else{
              return true;
            }    
    ...
    if(i18n.language === 'en') {
      setStateRequests('All Requests');
    }else{
      setStateRequests('Todas las solicitudes');
    }
  }
  ...
  useEffect(() => {
    ...
    dispatch(getRequestsByCooperative(cooperative.id));
  }, [dispatch]);
  ...
  return (
    <Page title={t('titleRequests')}>
      
      <Container>
        <HeaderBreadcrumbs
            heading={t('titleRequestsUser')}
            sx={{ mb: 0 }}
            links={[
              { name: t('inputs.home'), href: '/dashboard/main' },
              { name: t('inputs.membersManagement'), href: '/dashboard/main/members' },
              { name: t('titleRequests'), href: '/dashboard/main/myFarms' },
            ]}
          />
        <Card>
          <RequestsListToolbar
            ...
            {isLoading ? (
              ...
              filtered.length === 0 && !filterName ? (
                <Alert severity="info" sx={{ mt: 2 }}>
                   { stateRequests === 'Todas las solicitudes' ||  stateRequests === 'All Requests' ? t('inputs.notFoundRequestsCooperative') : t('inputs.notFound2RequestsCooperative') } 
                </Alert>
               ) : (
              <TableContainer sx={{ minWidth: 800 }}>
                <Table stickyHeader>
                  ...
                  <TableBody>
                    ...
                    filtered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                      ...
                      const { id, messageBody, status, user, createdAt } = row;
                      ...
                      return (
                        <TableRow hover key={id} tabIndex={-1}>
                          ...
                          <TableCell align="left" sx={{ width:'20%'}}>{user.name} {user.lastname} <br></br> {user.email} </TableCell>
               ...
               component="div"
               count={filteredRequestsCooperative.length}
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return array.filter(invites => (invites.user.name+" "+invites.user.lastname).toLowerCase().indexOf(query.toLowerCase()) !== -1);
  } 
