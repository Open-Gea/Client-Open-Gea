...
// actions
import { handleChangeRowsPerPage, setPage } from '../../../../redux/slices/invitesCooperativa';
import { getInvitesByCooperative } from './utils/getInvites';
...
export function InvitesInformation() {
  ...
  const { page, rowsPerPage, order, orderBy, filterName, filteredInvites, isLoading, error } = useSelector(state => state.invitesCooperativaSlice);
  const { cooperative } = useSelector(s => s.authSlice);
  const [showAlert, setShowAlert] = useState(false);
  const { enqueueSnackbar } = useSnackbar();
  const [stateInvites,setStateInvites] = useState('');


  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - filteredInvites.length) : 0;

  let filtered = applySortFilter(filteredInvites, getComparator(order, orderBy), filterName);

  if(stateInvites){
      // Filter by the state selected
      filtered = filtered.filter(function(invite){
          if(i18n.language === 'en') {
            if(stateInvites !== 'All Invites'){
              return  (invite.status[0].toLowerCase()) === (stateInvites[0].toLowerCase());
            }else{
              return true;
            }    
          }else{
            if(stateInvites !== 'Todas las invitaciones'){
              return (invite.status[0].toLowerCase()) === (stateInvites[0].toLowerCase());
            }else{
              return true;
            }    
    ...
    if(i18n.language === 'en') {
      setStateInvites('All Invites');
    }else{
      setStateInvites('Todas las invitaciones');
    }
  }
  ...
  useEffect(() => {
    ...
    // Here we need to load the invites by the cooperative
    dispatch(getInvitesByCooperative(cooperative.id));
  }, [dispatch]);
  useEffect(() => {
    if (!isLoading && error) {
            ...
            {isLoading ? (
              ...
              filtered.length === 0 && !filterName ? (
                <Alert severity="info" sx={{ mt: 2 }}>
                   { stateInvites === 'Todas las invitaciones' ||  stateInvites === 'All Invites' ? t('inputs.notFound') : t('inputs.notFound2') } 
                </Alert>
               ) : (
...
// Change user.name if you need to change the value you are searching in the search bar and orderby to define the filtering
