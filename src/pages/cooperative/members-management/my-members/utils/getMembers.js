...
import {
  ...
  getMembersSuccess,
} from '../../../../../redux/slices/membersCooperativa';
...
export const getUsersByCooperative = id => {
  return async dispatch => {
    ...
    try {
      // Bringing the data from the cooperative
      const responseCooperatives = await axios.get(`${import.meta.env.VITE_YVY_BACKEND_BASE_URL}/users/`);
      // Creating the adapted users
      const adapatedUsers = responseCooperatives.data.map(user => authAdapter(user));
...
export const getMembersByCooperative = id => {
  return async dispatch => {
    dispatch(startLoading);
    try {
      // Creating the list of countries
      ...
      // Adding the relation id to the users
      const adaptedUsersWithRelationId = adapatedUsers.map((obj, index) => {
        return {
          ...obj,
          user_coopId: responseCooperatives.data[index].id,
        };
      });

      dispatch(
        getMembersSuccess({
          members: adaptedUsersWithRelationId,
          uniqueCountries: countries,
        })
      );
    } catch (error) {
