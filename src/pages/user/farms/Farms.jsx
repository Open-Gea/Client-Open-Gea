...
// actions
import { deleteFarm, handleChangeRowsPerPage, setPage } from '../../../redux/slices/farms';
...
import { MoreInfoFarm } from './components/MoreInfoFarm';
...
export function Farms() {
  ...
  const { page, rowsPerPage, order, orderBy, filterName, filteredFarms, isLoading, error } = useSelector(state => state.farmSlice);
  ...
  const MyAlert = ({ message, onClose }) => {
    ...
    return (
      <Alert severity="error" onClose={onClose} sx={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 9999 }}>
        {message}
      </Alert>
    );
  };
                ...
                <Table stickyHeader>
                  ...
                  <TableBody>
                    {filtered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                      const { id, name, owner, phone, country, lat, lng } = row;
                      return (
                        <TableRow hover key={id} tabIndex={-1}>
                          ...
                          <Dialog
                            ...
                            sx={{ '& .MuiDialog-paper': { maxWidth: '90%', maxHeight: '90%' } }}
                          >
                              ...
                              <TableContainer sx={{ minWidth: 800, maxHeight: '70vh' }}>
                                <Table stickyHeader>
                                  ...
                                  <TableBody>
                                    {selectedRow &&
                                      selectedRow.urls &&
                                      selectedRow.urls.map(urlObj => (
                                        <TableRow key={urlObj.file_id} className="table-row">
                                          <TableCell sx={{ fontSize: '1.2rem', borderBottom: '1px solid #ddd' }}>
                                            {urlObj && urlObj.filename}
                                          </TableCell>
                                          <TableCell sx={{ fontSize: '1.2rem', borderBottom: '1px solid #ddd' }}>
                                            {urlObj && urlObj.fechaCarga}
                                          </TableCell>
