...
import { removeUserFromCooperative } from '../../../../../redux/slices/organizationsUser';
// redux
...
export default function OrganizationsMoreMenu({ row }) {
  ...
  const handleDecline = async (row)  => {
    await dispatch(removeUserFromCooperative(row.id));
    setDeclineConfirmationOpen(false);
    handleClose();
  };
  // ---------------------------
       ...
       <IconButton onClick={handleOpen} >
        ...
        >
          <MenuItem onClick={handleDeclineUserFromCooperative} sx={{ color: 'error.main' }}>
            <Iconify icon={'fluent-mdl2:leave-user'} sx={{ ...ICON }} />
            &nbsp; &nbsp;  {t('invitesTable.withdrawal')}
          </MenuItem>

        </MenuPopover>

        <Dialog open={declineConfirmationOpen} onClose={handleCloseDeclineConfirmation} key={row.id+'-decline'}>
            <DialogTitle> {t('menuPopover.confirmWithdrawal')}  </DialogTitle>
            <DialogContent> {t('menuPopover.confirmWithdrawalMsj1')}{row.cooperative.name}{t('menuPopover.confirmWithdrawalMsj2')}</DialogContent>
            <DialogActions>
...
OrganizationsMoreMenu.propTypes = {
