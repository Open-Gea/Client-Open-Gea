...
// actions
import { handleChangeRowsPerPage, setPage } from '../../../../redux/slices/requestsUser';
import { getRequestsByUser } from './utils/getRequests';
...
export function RequestsInformation() {
  ...
  const { page, rowsPerPage, order, orderBy, filterName, filteredRequests, isLoading, error } = useSelector(state => state.requestsUserSlice);
  ...
  let filtered = applySortFilter(filteredRequests, getComparator(order, orderBy), filterName);
  ...
  useEffect(() => {
    ...
    dispatch(getRequestsByUser(user.id));
  }, [dispatch]);
  useEffect(() => {
    if (!isLoading && error) {
            ...
            {isLoading ? (
              ...
              filtered.length === 0 && !filterName ? (
                <Alert severity="info" sx={{ mt: 2 }}>
                   { stateRequests === 'Todas las solicitudes' ||  stateRequests === 'All Requests' ? t('inputs.notFoundRequests') : t('inputs.notFound2Requests') } 
                </Alert>
               ) : (
                    ...
                    filtered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
                      ...
                      return (
                        <TableRow hover key={id} tabIndex={-1}>
                          ...
                          <TableCell align="left">{coop.name} <br></br> {coop.email} </TableCell>
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return array.filter(requests => (requests.coop.name).toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
