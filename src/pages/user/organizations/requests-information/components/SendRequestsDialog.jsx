...
import { getCooperativeByEmail, addRequest } from '../../../../../redux/slices/requestsUser';
...
export default function SendRequestsDialog({ onClose, open, title, edit, farmInfo }) {
  ...
  const LoginSchema = Yup.object().shape({
    userEmail: Yup.string().email(t('validations.validEmail')).required(t('validations.requiredEmailRequest')),
    bodyMessage: Yup.string().required(t('validations.requiredMessageRequest')),
  });

  const { user } = useSelector(state => state.authSlice);
  const defaultValues = { userEmail: '', bodyMessage: t('inputs.defaultMessageRequest') };
  ...
  const onSubmitAdd = async data => {
    ...
    try {
      ...
      dispatch(getCooperativeByEmail({ email: userEmail })).then(e => {
        if (e.error) {
          ...
          if(e.payload === ""){
            setError('userEmail', {
              message: t('validations.emailNotFoundRequest'),
            });
          }else{
            const dataToPost = {
            ...
            dispatch(addRequest(dataToPost));
            reset();
            onClose();
          }
        }
            ...
            <Grid item xs={12} md={10}>
              <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
                ...
                <Grid container spacing={1.5}>
                  <Grid item xs={12}>
                    <RHFTextField name="userEmail" label={t('inputs.cooperativeEmail')} />
                  </Grid>
                  <Grid item xs={12}>
                    <RHFTextField 
                      name="bodyMessage" 
                      multiline
                      label={t('inputs.bodyMessageRequest')}
                      type="text"
              ...
              <DialogContentText id="alert-dialog-slide-description">
                {t('inputs.formPlaceHolderMessageRequest')}
              </DialogContentText>
            </Grid>
          </Grid>
        </DialogContent>
