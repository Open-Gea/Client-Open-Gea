...
export default function ShowDetailsDialog({ details, fecha, factor }) {
  ...
  const data = [
    ...
    { title: t('calcTable.productLabel'), info: i18n.language === 'es' ? factor.name :  t('inputs.emissionFactors.'+factor.name)  },
    { title: t('calcTable.resultLabel'), info: details.result+' Kg CO2eq'  },
    { title: t('calcTable.categoryLabel'), info: i18n.language === 'es' ? factor.category :  t('inputs.categoryOptions.'+factor.category)   },
    { title: t('calcTable.yearLabel'), info: details.year },
    { title: t('calcTable.consumptionLabel'), info: i18n.language === 'en' ?  (factor.unit === 'Litros' || factor.unit === 'Personas/Año'  || factor.unit === 'Cabezas/Año') ? details.consumption+ ' ' + t('inputs.consumptionUnits.'+factor.unit) : details.consumption+ ' ' + factor.unit  :details.consumption+ ' ' + factor.unit },
